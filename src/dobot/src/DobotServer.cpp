...
{
    res.result = SetCmdTimeout(req.timeout);//输入时间，函数返回是否出错并把值赋予srv的result
...
void InitCmdTimeoutServices(ros::NodeHandle &n, std::vector<ros::ServiceServer> &serverVec)
{
    ...
    //创建一个名为“/DobotServer/SetCmdTimeout”的服务名称，并注册回调函数SetCmdTimeoutService
...
// 一系列获取设备信息的回调函数，在InitDeviceInfoServices中被注册。
bool GetDeviceSNService(dobot::GetDeviceSN::Request &req, dobot::GetDeviceSN::Response &res)
{
    char deviceSN[256];

    res.result = GetDeviceSN(deviceSN, sizeof(deviceSN));//该API的接口参数是存放设备信息的数组的指针。
...
{
    Pose pose;// 创建pose类型结构体，存放传感器读取的位姿信息
    ...
    if (res.result == DobotCommunicate_NoError) {
        ...
        res.r = pose.r;
        for (int i = 0; i < 4; i++) {
            res.jointAngle.push_back(pose.jointAngle[i]);//jointAngle是存放机械臂关节角的数组
        }
    }
...
{
    ROS_INFO("SetPTPService Calling!! \n");
    PTPCmd cmd;//创建结构体储存设置的坐标值
    uint64_t queuedCmdIndex;
    
    cmd.ptpMode = req.ptpMode;//客户端请求的数据
    cmd.x = req.x;
    cmd.y = req.y;
    cmd.z = req.z;
    cmd.r = req.r;
    res.result = SetPTPCmd(&cmd,1, &queuedCmdIndex);//改变了是否为队列模式
   
    cout<<res.result<<endl;

    // if (res.result == DobotCommunicate_NoError) {
    //     res.queuedCmdIndex = queuedCmdIndex;
    //     std::cout<<queuedCmdIndex<<std::endl;
    //     ROS_INFO("No Error!\n \n");
    // }
    // if(res.result == DobotCommunicate_Timeout)
    // {
    //     ROS_ERROR("指令无返回，导致超时！\n");
    // }
    // else(res.result = DobotCommunicate_BufferFull);
    // {
    //     ROS_ERROR("指令已满！\n");
    // }
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;//节点举句柄
    pointer_n = &n;

    //创建向量存放节点所有的注册服务对象。
    std::vector<ros::ServiceServer> serverVec;
//以下所有的函数功能都是创建相应的服务，并注册回调函数（回调函数已经被声明且实现），并将每一个service压入容器中。
  
    //指令超时结果判断,创建service通信。通过dobot的API找到返回结果。
    InitCmdTimeoutServices(n, serverVec);

    //初始化设备信息服务的函数，获取设备的版本名称和、SN。
